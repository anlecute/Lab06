...
public class StudentController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // Lấy tham số "action" từ URL, mặc định là "list" nếu null
        String action = request.getParameter("action");
        if (action == null) {
            action = "list"; // mặc định hiển thị danh sách
        }
        // Xử lý các action khác nhau dựa vào tham số
        try {
            switch (action) {
                case "new":
        ...
        // Nếu xảy ra lỗi, ném ServletException
        } catch (Exception e) {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String action = request.getParameter("action");
        // Phân loại action POST: insert hoặc update
    ...
    private void searchStudents(HttpServletRequest request, HttpServletResponse response)
        ...
        // Set các attribute để JSP hiển thị kết quả
        request.setAttribute("students", students);
   ...
   // Hiển thị form chỉnh sửa sinh viên
    private void showEditForm(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        int id = Integer.parseInt(request.getParameter("id")); // Lấy id sinh viên cần edit
    ...
    private void insertStudent(HttpServletRequest request, HttpServletResponse response)
        ...
        Student newStudent = new Student(studentCode, username, fullName, email, major, password, role);
        ...
        if (success) {
            // Nếu thành công, redirect về list với thông báo
            response.sendRedirect("student?action=list&message=Student+added+successfully");
        } else {
            // Nếu thất bại, giữ lại form và thông báo lỗi
            request.setAttribute("student", newStudent);
    ...
    private void updateStudent(HttpServletRequest request, HttpServletResponse response)
        ...
        Student student = new Student(studentCode, username, fullName, email, major, password, role);
        student.setId(id);
...
//Trong student-form.jsp, dùng một form chung cho cả thêm mới (Add) và chỉnh sửa (Edit) sinh viên.
//Khi thêm mới (Add), form phải gửi action = insert
//Khi chỉnh sửa (Edit), form phải gửi action = update
//
//request.setAttribute("formAction", "insert");
//
//<form action="student" method="post">
//    <input type="hidden" name="action" value="${formAction}">
//    <!-- các input khác: studentCode, fullName, email, major -->
//</form>

//${formAction} được set từ Servlet:
//
//Nếu bạn gọi showNewForm(), thì request.setAttribute("formAction", "insert")
//form sẽ gửi action=insert khi submit
//
//Nếu bạn gọi showEditForm(), thì request.setAttribute("formAction", "update")
//form sẽ gửi action=update khi submit